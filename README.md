...
* `GAP_WORKERS`: used by `generate_rrm_v11_parallel.sh`. Default `1` runs sequential `generate_rrm` (byte-identical to invoking GAP on `generate_rrm_v11_fast.g` directly). For `k>1`, the master writes `vertices_*.dat` and runs the Pechukas checks; each worker writes an edge shard for a contiguous TS-index slice; shards are concatenated in TS-index order. Workers print `RRM_NVERT`; the driver aborts if a worker's count differs from the master's. `MEM` (default `12g`) is per GAP process, so `k` workers plus the master request about `(k+1)` times that memory.
